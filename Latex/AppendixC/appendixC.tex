...
The failure rate is given by the following relationship
$$\lambda = \lambda{b} \pi_{CV} \pi_{Q} \pi_{E} \frac{\text{failures}}{10^{6} \text{hours}}$$

For all factors $T$ = ambient temperature ($^\circ$C) and 
$S=\frac{\text{operating voltage}}{\text{rated voltage}}$, where the
operating voltage is the sum of the DC and peak AC voltage.

\begin{table}[h]
\caption{\textbf{Base Failure Rate -} $\lambda_{b}$}
\label{table:baseFailureRateCapacitor}
\begin{tabular}{l}
\T $\lambda_b = 0.0003 [(\frac{S}{0.3}^{3})+1] exp [\frac{T+273}{358}]$ for T=85$^{\circ}$C max rated  \B \\ 
\T $\lambda_b = 0.0003 [(\frac{S}{0.3}^{3})+1] exp [\frac{T+273}{398}]$ for T=125$^{\circ}$C max rated \B \\ 
\T $\lambda_b = 0.0003 [(\frac{S}{0.3}^{3})+1] exp [\frac{T+273}{423}]$ for T=150$^{\circ}$C max rated \B \\ 
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{\textbf{Quality Factor -} $\pi_{Q}$}
\label{table:capacitanceQualityFactor}
\begin{tabular}{|c|c|} \hline
\rowcolor{Gray}
\textbf{Quality} & $\pi_{Q}$ \\ \hline
S & 0.030 \\ \hline
...
$$\lambda = \lambda_{b} \pi_{T} \pi_{A} \pi_{Q} \pi_{E} \frac{\text{failures}}{10^{6} \text{hours}}$$
...
\begin{table}[h]
\caption{\textbf{Application Factor -}  $pi_{A}$}
\label{table:applicationFailRateMosLow}
\begin{tabular}{|c|c|} \hline
\rowcolor{Gray}
...
\begin{table}[h]
\caption{\textbf{Part Quality  Factor -}  $pi_{Q}$}
\label{table:qualityFactorFailRateMosHigh}
\begin{tabular}{|c|c|} \hline
\rowcolor{Gray}
\textbf{Quality} & $\pi_{Q}$ \\ \hline
JANTXV & 0.5 \\ \hline
